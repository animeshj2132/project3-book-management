...
const createReview = async function (req, res) {
    try {
        ...
        let check = Object.keys(content)
        let arr = ['reviewedBy', 'review', 'rating', 'reviewedAt']

        for (let i = 0; i < check.length; i++) {

            let update = arr.includes(check[i])
            if (!update) return res.status(400).send({ status: false, msg: "you can only create a review by giving these fields: review,rating reviewedBy and reviewedAt ." })
        }
        ...
        if (rAt && rat && rev) {
            ...
            if (!isValidDate(rAt)) { return res.status(400).send({ status: false, message: "please enter a valid date " }) }


            if (!isBody(rev)) { return res.status(400).send({ status: false, message: "please enter review correctly" }) }
...
if (!reviewedByValidator(body.reviewedBy)) { return res.status(400).send({ status: false, message: "please enter reviewedBy correctly" }) }

        if (!isBody(body.review)) { return res.status(400).send({ status: false, message: "please enter review correctly" }) }

        if (!validateRating(body.rating)) { return res.status(400).send({ status: false, message: "please enter a rating between 1 to 5 " }) }
